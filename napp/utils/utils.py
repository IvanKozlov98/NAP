...
def divide_by_distance(matrix, resolution, upto=1000000):
    ...
    :return: two dim numpy array - a[i] ->
    [count between i1 bin and i2 bin with distance i * resolution]
    """
    n, m = matrix.shape
    assert n == m
...
def get_auc_distance_stratified_correlation(matrix_1, matrix_2, resolution):
    ...
    :param matrix_2:
    :param resolution: size of bin
    :return: summarized the distance-stratified
    Pearson’s correlation curve into the area under the curve
    """
...
def get_random_diag_matrix(size, max_val, std):
    """Random matrix in which diagonal elements more than other"""
